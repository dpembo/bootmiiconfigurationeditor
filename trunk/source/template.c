...
// (C)2009 Pembo - http://www.pembo.co.uk
//
// ==========================================================
...
void writeFile()
{
	...
	FILE *f = fopen ("SD:/bootmii/bootmii.ini", "wb");
	if (f == NULL) 
	{
...
{
	char* value;
	FILE *f = fopen ("SD:/bootmii/bootmii.ini", "rb");
	if(f==NULL)
	{
...
void drawHeader()
{
	...
	//drawString(2,8,GREEN,GRAY, "(C)2009 Pembo - http://www.pembo.co.uk");
...
void drawMenu(int onPos,int prevPos)
{
	...
	if(onPos ==4)col4=skin_main_selectedIndicatorcolor;
	...
	drawString(optionsPos,MAIN_WINY+6,col4,skin_main_bgcolor,strdup(mm5));
	drawString(optionsPos,MAIN_WINY+7,col5,skin_main_bgcolor,strdup(mm6));
}
...
void initScreen()
{
	...
	drawWindow (4,MAIN_WINY,15,72,skin_main_bgcolor,skin_main_titlecolor,skin_main_titlelinecolor,skin_main_bgcolor,skin_main_shadow,skin_main_decl,"BootMii Configuration Editor (v2.4)");
	drawString(37,MAIN_WINY + 13,skin_main_textcolor2,skin_main_bgcolor, "(C)2009 Pembo - http://www.pembo.co.uk");
...
void initialiseCoreApplication(int argc, char* argv)
{
	...
	printf("\n\n Initialising BootMii Configuration Editor (v2.4) - Please wait\n\n");
	//sleep(5);
...
boolean updateFileFromUrl(char* url , char* location)
{
	printf("\n");
...
void checkForUpdatedVersion()
{
			...
			if ((result.data[0] >= VERSION) && (result.data[1] > REV))
			{
...
int main(int argc, char *argv[])
{
	//printf("initialise JPEG");
	//sleep (1);
	memset(&title, 0, sizeof(JPEGIMG));
	title.inbuffer = picdata;
	title.inbufferlength = piclength;
	JPEG_Decompress(&title);
	//sleep(5);
